...
#[test]
fn asset_bundle_normalize_rewrites_to_canonical_json() {
    let input = write_temp_value(
        ...
        &serde_json::json!({
            ...
            "snapshots": [
                {
                    "id": "zeta",
                    "path": fixture("fixtures/contracts/snapshot.valid.json").to_str().expect("utf-8 path"),
                },
                {
                    "id": "alpha",
                    "path": fixture("fixtures/contracts/snapshot.valid.json").to_str().expect("utf-8 path"),
    ...
    let output = run_cli(&["asset-bundle-normalize", input.to_str().expect("utf-8 path")]);
    ...
    assert_eq!(
        normalized["catalog"],
        serde_json::json!(fixture("fixtures/contracts/program.valid.json")
            .to_str()
            .expect("utf-8 path"))
    );
    assert_eq!(normalized["snapshots"][0]["id"], "alpha");
    assert_eq!(normalized["snapshots"][1]["id"], "zeta");
...
#[test]
fn asset_bundle_normalize_preserves_validation_compatibility() {
    let input = write_temp_value(
        "asset-bundle-normalize-validity",
        &serde_json::json!({
            "version": 1,
            "catalog": fixture("fixtures/contracts/program.valid.json").to_str().expect("utf-8 path"),
            "snapshots": [
    ...
    let output_path = write_temp_json("asset-bundle-normalize-output", "{}");
    ...
    let check = run_cli(&["asset-bundle-check", output_path.to_str().expect("utf-8 path")]);
    assert!(check.status.success(), "stderr={}", String::from_utf8_lossy(&check.stderr));

    let normalized: Value =
        serde_json::from_str(&fs::read_to_string(&output_path).expect("read normalized output"))
            .expect("parse normalized asset bundle");
    assert_eq!(normalized["snapshots"][0]["id"], "alpha");
    assert_eq!(normalized["snapshots"][1]["id"], "beta");
